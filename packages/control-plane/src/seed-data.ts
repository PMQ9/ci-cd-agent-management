...
export const SEED_PROMPTS: SeedPrompt[] = [
  ...
  },
  {
    ...
    content:
      'You MUST structure your entire review to fill the REVIEW TEMPLATE below — do not freestyle, do not omit sections, do not invent new ones. Map every issue you find into the template\'s structure with an appropriate severity. Write "None." for any section that has nothing to report. In suggested fixes, reference file:line where possible and list only the highest-leverage actions.\n\nEmit your review as the JSON object described at the very end of this prompt. Its fields map onto the template: "summary" + "verdict" = the Review summary, "findings[]" (each with a severity) = the Findings section (bucketed by severity when rendered), "concerns[]" = Concerns, "suggestedFixes[]" = Suggested fixes. Output ONLY that JSON object — never the markdown template itself, never prose around it.',
    editable: true,
  },
  {
    ...
    content:
      "This is a RE-REVIEW. The previous round's findings are listed below. For each prior finding, decide whether it is now resolved: only re-report findings that are still open, and ADD any new regressions introduced since the last round. Keep the same template structure.",
    editable: true,
  },
];
